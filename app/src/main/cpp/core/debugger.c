...
static BOOL    OnMemMap(HWND hDlg);
static VOID    UpdateProfileWnd(HWND hDlg);
static VOID    UpdateMemMapWnd(HWND hDlg);
...
static VOID StrToReg(BYTE *pReg, WORD wNib, LPTSTR lpszValue)
{
	...
	for (i = wNib - 1;i >= 0;--i)
	{
		...
		{
			// convert to number
			pReg[i] = (BYTE) _totupper(*lpszValue) - _T('0');
...
VOID OnUpdate(HWND hDlg)
{
	...
	dwDbgStopPC = (DWORD) -1;				// disable "cursor stop address"
	...
	UpdateMemMapWnd(hDlgMemMap);			// update memory mapping dialog
...
{
	DWORD dwAddress = (DWORD) -1;			// no address given
...
static BOOL OnLButtonUp(HWND hDlg, LPARAM lParam)
{
	...
	case IDC_REG_P: // P
		...
		Chipset.P = (BYTE) _totupper(szBuffer[2]) - _T('0');
...
static INT_PTR CALLBACK Debugger(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendMessage(hDlg,WM_SETICON,ICON_BIG,(LPARAM) LoadIcon(hApp,MAKEINTRESOURCE(IDI_EMU71)));
		...
		pdwInstrArray = (LPDWORD) malloc(wInstrSize*sizeof(*pdwInstrArray));
		...
		dwDbgStopPC = (DWORD) -1;			// no stop address for goto cursor
	...
	case WM_COMMAND:
		...
		case ID_INFO_MEMMAP:              return OnMemMap(hDlg);
		...
		case ID_DEBUG_MEM_MAP:
		case ID_DEBUG_MEM_ROM:            return OnMemMapping(hDlg,LOWORD(wParam));
...
static INT_PTR CALLBACK EditBreakpoint(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_BREAKEDIT_ADD:
			sBp.dwAddr = (DWORD) -1;		// no breakpoint given
...
static INT_PTR CALLBACK InfoIntr(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		for (i = wInstrRp; i != wInstrWp; i = (i + 1) % wInstrSize)
		{
			...
			j = (WORD) wsprintf(szBuffer,_T("%05lX   "),pdwInstrArray[i]);
...
// add text leaf
//
static HTREEITEM __cdecl AddLeaf(HWND hTreeWnd, HTREEITEM hti, LPCTSTR lpFormat, ...)
{
	...
	tvins.item.pszText = cOutput;
	return TreeView_InsertItem(hTreeWnd,&tvins);
}

//
// add internal display driver chip
//
static VOID AddDisplayDriver(HWND hTreeWnd, HTREEITEM hti, LPTSTR lpszText, DWORD dwIOBase, DWORD dwRamBase)
{
	DWORD s,e;

	hti = AddLeaf(hTreeWnd,hti,lpszText);

	// I/O section
	s = dwIOBase & (MAPMASK(ARRAYSIZEOF(Chipset.dd[0].IORam)) << ADDR_BITS);
	e = s + (MAPSIZE(ARRAYSIZEOF(Chipset.dd[0].IORam) + 1) << ADDR_BITS) - 1;
	AddLeaf(hTreeWnd,hti,_T("I/O: %05X - %05X"),s,e);

	// RAM section
	s = dwRamBase & (MAPMASK(ARRAYSIZEOF(Chipset.dd[0].Ram)) << ADDR_BITS);
	e = s + (MAPSIZE(ARRAYSIZEOF(Chipset.dd[0].Ram) + 1) << ADDR_BITS) - 1;
	AddLeaf(hTreeWnd,hti,_T("RAM: %05X - %05X"),s,e);

	TreeView_Expand(hTreeWnd,hti,TVE_EXPAND);
	return;
}

//
// add internal RAM chip
//
static VOID AddIntRAM(HWND hTreeWnd, HTREEITEM hti, LPTSTR lpszText, C1LG8 *psChip)
{
	INT i;

	hti = AddLeaf(hTreeWnd,hti,_T("%s (%uK)"),lpszText,ARRAYSIZEOF(psChip->Ram) / 2048);

	for (i = 0; i < ARRAYSIZEOF(psChip->sCfg); ++i)
	{
		if (psChip->sCfg[i].bCfg == FALSE)
		{
			AddLeaf(hTreeWnd,hti,_T("Unconfigured: Size = %05X"),
				ARRAYSIZEOF(psChip->Ram) / ARRAYSIZEOF(psChip->sCfg));
		}
		else
		{
			AddLeaf(hTreeWnd,hti,_T("Configured: %05X - %05X"),
				psChip->sCfg[i].dwBase,
				psChip->sCfg[i].dwBase+ARRAYSIZEOF(psChip->Ram) / ARRAYSIZEOF(psChip->sCfg) - 1);
		}
	}
	TreeView_Expand(hTreeWnd,hti,TVE_EXPAND);
	return;
...
// add port chip
//
static VOID AddPortChip(HWND hTreeWnd, HTREEITEM hti, PPORTDATA psData)
{
	LPCTSTR lpszType;
	DWORD   dwSize;
	UINT    i;

	dwSize = psData->sInfo.dwSize;			// this is the regular module size
	...
	case TYPE_ROM:
		...
		break;
	case TYPE_HPIL:
		lpszType = _T("HPIL I/O");
		// replace module size with mailbox interface size
		dwSize = ARRAYSIZEOF(((PHPILDATA)0)->byIOMem);
		break;
	default: _ASSERT(FALSE);
		lpszType = _T("");
	}
	...
	if (dwSize < 2048)
		hti = AddLeaf(hTreeWnd,hti,_T("%s (%u)"), lpszType,dwSize / 2);
	else
		hti = AddLeaf(hTreeWnd,hti,_T("%s (%uK)"),lpszType,dwSize / 2048);

	// the configuration
	for (i = 0; i < psData->sInfo.dwChips; ++i)
	{
		if (psData->psCfg[i].bCfg == FALSE)
		{
			AddLeaf(hTreeWnd,hti,_T("Unconfigured: Size = %05X"),dwSize / psData->sInfo.dwChips);
		}
		else
		{
			AddLeaf(hTreeWnd,hti,_T("Configured: %05X - %05X"),
					psData->psCfg[i].dwBase,
					psData->psCfg[i].dwBase + (dwSize / psData->sInfo.dwChips) - 1);
		}
	}
...
//
static VOID ShowMemCfgTree(HWND hTreeWnd)
{
	HTREEITEM htiRoot,htiLeaf;
	UINT i;

	// add root of tree
	htiRoot = AddLeaf(hTreeWnd,TVI_ROOT,_T("HP-71B"));

	// add the main ROM and the display driver chip manually
	{
		HTREEITEM htiInfo;

		// base address of hard programmed modules
		CONST DWORD dwIOBase[]  = { 0x2E100, 0x2E200, 0x2E300 };
		CONST DWORD dwRamBase[] = { 0x2F400, 0x2F800, 0x2FC00 };

		_ASSERT(ARRAYSIZEOF(dwIOBase) == ARRAYSIZEOF(dwRamBase));

		htiLeaf = AddLeaf(hTreeWnd,htiRoot,_T("Internal"));

		if (Chipset.bOD == FALSE)			// ROM enabled
		{
			// the main ROM
			htiInfo = AddLeaf(hTreeWnd,htiLeaf,_T("ROM (%uK)"),dwRomSize / 2048);
			for (i = 0; i < dwRomSize; i += 0x08000)
			{
				AddLeaf(hTreeWnd,htiInfo,_T("%05X - %05X"),i,i + 0x08000 - 1);
			}
			TreeView_Expand(hTreeWnd,htiInfo,TVE_EXPAND);
		}

		// the display driver
		for (i = 0; i < ARRAYSIZEOF(dwIOBase); ++i)
		{
			// add chip
			AddDisplayDriver(hTreeWnd,htiLeaf,_T("Display Driver"),dwIOBase[i],dwRamBase[i]);
		}
		TreeView_Expand(hTreeWnd,htiLeaf,TVE_EXPAND);
	}

	// scan through loaded ports
	for (i = 0; i < ARRAYSIZEOF(psExtPortData); ++i)
	{
		...
		if (i == 0 || psData != NULL)
		{
			htiLeaf = AddLeaf(hTreeWnd,htiRoot,_T("Port%u"),i);

			if (i == 0)						// port0
			{
				int j;

				// add the internal RAM chips to Port0
				for (j = 0; j < ARRAYSIZEOF(Chipset.ir); ++j)
				{
					AddIntRAM(hTreeWnd,htiLeaf,_T("RAM"),&Chipset.ir[j]);
				}
			}

			// walk through all chips attached to the port
			for (; psData != NULL; psData = psData->pNext)
			{
				AddPortChip(hTreeWnd,htiLeaf,psData);
			}
		}
		TreeView_Expand(hTreeWnd,htiLeaf,TVE_EXPAND);
	}
	TreeView_Expand(hTreeWnd,htiRoot,TVE_EXPAND);
	return;
}
...
//
static HTREEITEM GetExpMemCfgTree(CONST HWND hTreeWnd,HTREEITEM htiParent,PWORD *ppwData)
{
	TVITEM tvi;
	HTREEITEM htiChild;

	// ppwData[0] = buffer size
	// ppwData[1] = index, last data position
	// ppwData[2] = level
	...
	do
	{
		...
		#if TVIS_EXPANDED != (1 << 5)
			#error "Wrong shift operator for TVIS_EXPANDED"
		#endif
		wExpand = (WORD) ((tvi.state & TVIS_EXPANDED) >> 5);

		htiChild = TreeView_GetChild(hTreeWnd,htiParent);
		if (htiChild)
		{
			// save only the state of nodes
			if (++(*ppwData)[1] >= (*ppwData)[0])
			{
				(*ppwData)[0] += 256;		// allocate additional entries
				*ppwData = (PWORD) realloc(*ppwData,(*ppwData)[0] * sizeof(**ppwData));
			}
			(*ppwData)[(*ppwData)[1]] = ((*ppwData)[2] << 1) | wExpand;

			++(*ppwData)[2];				// next level
			htiParent = GetExpMemCfgTree(hTreeWnd, htiChild, ppwData);
		}
		else
		{
			do
			{
				htiChild = TreeView_GetNextSibling(hTreeWnd,htiParent);
				if (htiChild == NULL)
				{
					--(*ppwData)[2];		// previous level
					htiParent = TreeView_GetParent(hTreeWnd,htiParent);
				}
			}
...
//
static VOID SetExpMemCfgTree(CONST HWND hTreeWnd,HTREEITEM hti,WORD CONST * CONST pwData)
{
	UINT uIndex;

	for (uIndex = 3; uIndex <= pwData[1];)
	{
		WORD wLevel = pwData[uIndex];		// get node info

		// update node state
		#if TVE_COLLAPSE != 0x0001 || TVE_EXPAND != 0x0002
			#error "Wrong constants for TVE_COLLAPSE or TVE_EXPAND"
		#endif
		TreeView_Expand(hTreeWnd,hti,(wLevel & 0x1) + 1);

		wLevel >>= 1;						// actual level

		if (++uIndex <= pwData[1])			// next level entry available
		{
			WORD wNextLevel = pwData[uIndex] >> 1;

			// navigate through the tree over the level code
			if (wNextLevel < wLevel)		// level up
			{
				hti = TreeView_GetParent(hTreeWnd,hti);
				hti = TreeView_GetNextSibling(hTreeWnd,hti);
			}
			else if (wNextLevel == wLevel)	// same level
			{
				hti = TreeView_GetNextSibling(hTreeWnd,hti);
			}
			else if (wNextLevel > wLevel)	// level down
			{
				hti = TreeView_GetChild(hTreeWnd,hti);
			}
		}
	}
	return;
...
//
// resize client tree window
//
static BOOL ResizeMapWindow(HWND hDlg,HWND hWnd)
{
	RECT rc;
...
static VOID UpdateMemMapWnd(HWND hDlg)
{
	if (hDlg != NULL && bBusCfg)			// dialog is open and bus configuration changed
	{
		HWND hWnd = GetDlgItem(hDlg,IDC_DEBUG_MEMCFG_TREE);

		// get actual position of scroll bar
		int nScPos = GetScrollPos(hWnd,SB_VERT);

		CONST UINT uBuffersize = 256;
		PWORD pwData = (PWORD) malloc(uBuffersize * sizeof(*pwData));

		_ASSERT(uBuffersize >= 3);			// for preset
		pwData[0] = uBuffersize;			// buffer size
		pwData[1] = 2;						// index, last data position
		pwData[2] = 0;						// level

		SendMessage(hWnd,WM_SETREDRAW,FALSE,0);

		// get expanded/collapsed state of nodes
		GetExpMemCfgTree(hWnd,TreeView_GetRoot(hWnd),&pwData);

		TreeView_DeleteAllItems(hWnd);		// delete tree
		ShowMemCfgTree(hWnd);				// rebuild tree

		// restore state of nodes
		SetExpMemCfgTree(hWnd,TreeView_GetRoot(hWnd),pwData);
		free(pwData);

		SetScrollPos(hWnd,SB_VERT,nScPos,FALSE);
...
//
static INT_PTR CALLBACK MemMap(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	HWND hWnd = GetDlgItem(hDlg,IDC_DEBUG_MEMCFG_TREE);
	...
	case WM_INITDIALOG:
		SendMessage(hDlg,WM_SETICON,ICON_BIG,(LPARAM) LoadIcon(hApp,MAKEINTRESOURCE(IDI_EMU71)));
		ShowMemCfgTree(hWnd);				// build the tree
		return TRUE;
	...
	case WM_SIZE:
		return ResizeMapWindow(hDlg,hWnd);
...
static BOOL OnMemMap(HWND hDlg)
{
	if (hDlgMemMap == NULL)				// no memory mapping dialog, create it
	{
		if ((hDlgMemMap = CreateDialog(hApp,MAKEINTRESOURCE(IDD_DEBUG_MEMMAP),hDlg,
			(DLGPROC)MemMap)) == NULL)
			AbortMessage(_T("Memory Mapping Dialog Box Creation Error !"));
	}
	else
	{
		SetFocus(hDlgMemMap);				// set focus on memory mapping dialog
	}
	return -1;								// call windows default handler
}
...
VOID LoadBreakpointList(HANDLE hFile)		// NULL = clear breakpoint list
{
	...
	// breakpoints found
	if (lBytesRead == sizeof(wBreakpointCount) && lBytesRead < ARRAYSIZEOF(sBreakpoint))
	{
