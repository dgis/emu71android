...
//	w.cycles+=6;
	...
	// look at HP71 IDS 3.6 CPU Power-down; Wake -up

	// If a SHUTDN instruction is
	// executed with the least significant bit of the Output Register
	// (ORO) set at a "0" the CPU will wake-up immediately, set its PC to
	// zero, assert hex mode arithmetic, and send out the PC. This avoids
	// a situation where the CPU is shutdown and the keyboard cannot wake
	// it up.
	if ((w.out & 0x1) == 0)					// OR0 is low
	{
		w.pc = 0;
		w.mode_dec = FALSE;
		return;
...
VOID o80E(LPBYTE I) // SREQ?
{
	...
	// HP-IL Mailbox responds with BUS[1] = high
	// Card Reader   responds with BUS[2] = high
	w.C[0] = SREQ();
	if (w.C[0]) w.HST |= SR;
	return;
}
