...
DWORD  dwSaturnCycles = 38;					// Saturn cpu cycles in interval
...
DWORD  dwDbgStopPC = (DWORD) -1;			// stop address for goto cursor
...
static DWORD dwEDbgT[] = { 0, 0, 0 };		// debugger timer emulation
static DWORD dwEDbgCycles[] = { 0, 0, 0 };	// debugger cycle counter

static BOOL  bBusCfgOpc = FALSE;			// device configuration on current opcode
...
static __inline VOID Debugger(VOID)			// debugger part
{
	...
	// device configuration opcode occured
	bBusCfg = bBusCfg || bBusCfgOpc;

	// CONFIG opcode #805, UNCNFG opcode #804 or RESET opcode #80A
	bBusCfgOpc = (I[0] == 0x8 && I[1] == 0x0 && (I[2] == 0x4 || I[2] == 0x5 || I[2] == 0xA));
	...
	if (bStopEmulation)						// stop condition
	{
		...
		for (i = 0; i < ARRAYSIZEOF(Chipset.dd); ++i)
		{
			...
			{
				dwTimerVal = Npack(&Chipset.dd[i].IORam[TIMER1 & 0xFF],DCONTR-TIMER1);
			}
			LeaveCriticalSection(&csTLock);

			_ASSERT(i < ARRAYSIZEOF(dwEDbgT));
			_ASSERT(i < ARRAYSIZEOF(dwEDbgCycles));

			if (dwEDbgT[i] == dwTimerVal)
			{
				// cpu cycles for one timer tick elapsed
				if ((DWORD) (Chipset.cycles & 0xFFFFFFFF) - dwEDbgCycles[i]
					>= (SAMPLE / 512) * (DWORD) T2CYCLES)
				{
					// new timer value
					dwTimerVal = (dwTimerVal - 1) & 0x00FFFFFF;

					EnterCriticalSection(&csTLock);
					{
						// write back to register
						Nunpack(&Chipset.dd[i].IORam[TIMER1 & 0xFF],dwTimerVal,DCONTR-TIMER1);
					}
					LeaveCriticalSection(&csTLock);

					// adjust cycles reference
					dwEDbgCycles[i] += (SAMPLE / 512) * T2CYCLES;
				}
			}
			else							// new master timer2 value
			{
				// new cycle reference
				dwEDbgCycles[i] = (DWORD) (Chipset.cycles & 0xFFFFFFFF);
			}
			dwEDbgT[i] = dwTimerVal;		// master timer check reference value
		}
		...
		bBusCfg = FALSE;					// device configuration updated
