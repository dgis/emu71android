...
static BYTE WaveLevel(WORD wOut)
{
	...
	switch (wOut)							// decode volume level
	{
	case 0 << 10: // off
		byWaveLevel = 0;
		break;
	case 1 << 10:
	case 3 << 10: // loud
		byWaveLevel = (BYTE) (dwWaveVol & 0xFF);
		break;
	case 2 << 10: // quiet
		byWaveLevel = (BYTE) ((dwWaveVol / 6) & 0xFF);
		break;
	default:
		_ASSERT(FALSE);
	}
...
VOID SoundOut(CHIPSET* w, WORD wOut)
{
	...
	if (dwDiffCycles > dwCpuFreq / 2)		// frequency < 1 Hz
	...
	byWaveLevel = 0x80 + (WaveLevel(wOut) - WaveLevel(w->out)) / 2;
	...
	if (w->bOD == FALSE)					// don't disable speed slow down with DIAG module
	{
